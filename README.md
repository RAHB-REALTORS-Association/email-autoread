...
```bash
docker pull ghcr.io/rahb-realtors-association/email-autoread:master
```
...
```bash
docker run -e OPENAI_API_KEY=<your_openai_api_key> -e USE_LOCAL=<true_or_false> -v /path/to/your/settings.json:/app/settings.json ghcr.io/rahb-realtors-association/email-autoread:master
```
